...
<div class="legend">
    ...
    <div style="margin-left:auto;display:flex;align-items:center;gap:12px;">
        <span style="font-size:11px;color:#999;">Tap any cell to edit score</span>
    </div>
</div>
...
// --- Override system (Vercel KV) ---
var scoreOverrides = {{}};
var _popupPlayer = '', _popupTeam = '';
...
async function saveScore(score) {{
    ...
    try {{
        await fetch('/api/overrides', {{
            ...
            body: JSON.stringify({{ player, team, score }})
        }});
        if (score === null) {{
            delete scoreOverrides[player + '|' + team];
        }} else {{
            scoreOverrides[player + '|' + team] = score;
        }}
    }} catch(e) {{ alert('Failed to save override'); }}
    renderMatrix();
}}

function openScorePopup(player, team, event) {{
    _popupPlayer = player;
    _popupTeam = team;
    document.getElementById('popupTitle').textContent = player + ' \\u2014 ' + team;
...
function buildMatrix() {{
    ...
    Object.values(latest).forEach(r => {{
        ...
        const ok = r.player + '|' + r.team;
        const score = scoreOverrides.hasOwnProperty(ok) ? scoreOverrides[ok] : r.score;
...
function renderMatrix() {{
    ...
    sortedPlayers.forEach(player => {{
        ...
        ALL_TEAMS.forEach(team => {{
            ...
            const ok = player + '|' + team;
            const isOverridden = scoreOverrides.hasOwnProperty(ok);
            if (s !== undefined && s !== null) {{
                sHtml += '<td class="' + scoreClass(s) + ' score-cell clickable' + (isOverridden ? ' overridden' : '') + '" onclick="openScorePopup(\\'' + esc + '\\', \\'' + team + '\\', event)">' + s + '</td>';
            }} else {{
                sHtml += '<td class="score-cell clickable" onclick="openScorePopup(\\'' + esc + '\\', \\'' + team + '\\', event)"></td>';
            }}
        }});
        sHtml += '</tr>';
    }});
