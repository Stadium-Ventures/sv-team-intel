...
async function exportCalendarPDF() {{
    ...
    // Render in-viewport but invisible. html2canvas clones into an iframe and has trouble
    // with `position:fixed; left:-10000px` (appears as blank output) — keeping the element
    // inside the document flow at (0,0) with opacity:0 + pointer-events:none avoids that.
    container.style.cssText = 'position:absolute;top:0;left:0;width:1100px;background:white;padding:26px 28px;font-family:Arial,sans-serif;color:#222;opacity:0;pointer-events:none;z-index:-1;';
    container.innerHTML =
...
// Keep the collapsed mrScore select tinted to match its current value.
// Chrome ignores `<option>` backgrounds on the closed select; coloring the select
// element directly via JS is the reliable cross-browser path.
const _MR_SCORE_COLORS = {{
    '2':  {{ bg: '#c6efce', fg: '#1a5e1a', weight: 700 }},
    '1':  {{ bg: '#e2efda', fg: '#3a6b30', weight: 600 }},
    '0':  {{ bg: '#fff2cc', fg: '#7a6b00', weight: 600 }},
    '-1': {{ bg: '#fce4ec', fg: '#9a2020', weight: 600 }},
    '-2': {{ bg: '#f4c7c3', fg: '#8b1a1a', weight: 700 }},
}};

function _mrSyncScoreColor() {{
    const sel = document.getElementById('mrScore');
    if (!sel) return;
    const c = _MR_SCORE_COLORS[sel.value];
    if (!c) return;
    sel.style.backgroundColor = c.bg;
    sel.style.color = c.fg;
    sel.style.fontWeight = c.weight;
}}
...
<div id="mrOverlay" onclick="if(event.target===this) closeManualEntryModal()">
    <div id="mrModal">
        ...
        <div class="ev-row">
            ...
            <select id="mrScore" onchange="_mrSyncScoreColor()">
                <option value="2" style="background:#c6efce;color:#1a5e1a;font-weight:700;">+2 (Strong Interest)</option>
                <option value="1" selected style="background:#e2efda;color:#3a6b30;font-weight:600;">+1 (Interest)</option>
                <option value="0" style="background:#fff2cc;color:#7a6b00;font-weight:600;">0 (Neutral)</option>
                <option value="-1" style="background:#fce4ec;color:#9a2020;font-weight:600;">-1 (Cool / No Contact)</option>
                <option value="-2" style="background:#f4c7c3;color:#8b1a1a;font-weight:700;">-2 (Negative)</option>
            </select>
        </div>
        <div class="ev-row">
